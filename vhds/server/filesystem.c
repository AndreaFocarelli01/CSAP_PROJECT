...
int fs_create(FsContext *ctx, const char *path, mode_t perm, int is_dir) {
    ...
    /* uid/gid dell'utente loggato — usati per chown dopo la creazione */
    const UserRecord *ur = session_find_user(ctx->sess_tbl,
                                              ctx->sess->username);
    uid_t owner_uid = ur ? (uid_t)ur->uid : (uid_t)-1;
    gid_t owner_gid = ur ? (gid_t)ur->gid : (gid_t)-1;

    int ret;
    if (is_dir) {
        sync_write_lock(ctx->lock_tbl, parent);
        ret = mkdir(abs, perm);
        if (ret == 0 && owner_uid != (uid_t)-1)
            chown(abs, owner_uid, owner_gid);
...
int fs_write(FsContext *ctx, const char *path, int offset, int append_mode) {
    ...
    if (!err && use_tmp) {
        ...
        if (rename(tmp_path, abs) < 0) {
            ...
            /* chown solo se il file è nuovo (non esisteva prima) */
            if (!exists) {
                const UserRecord *ur = session_find_user(ctx->sess_tbl,
                                                          ctx->sess->username);
                if (ur) chown(abs, (uid_t)ur->uid, (gid_t)ur->gid);
            }
